...
class Panel extends Object implements IBarPanel, Subscriber
{
	...
	public function getTab()
	{
		...
		. '<span class="tracy-label">' . count($this->requests) . ' queries' . ($this->totalTime ? ' / ' . sprintf('%0.1f', $this->totalTime) . 'ms' : '') . ' / ' . sprintf('%0.1f', $altTotalTime) . 'ms / ' . sprintf('%0.1f', $alt2TotalTime) . 'ms</span></span>';
	}
